...
│   ├── test_formiration_chirp_golden.vhd  # exhaustive SF5/SF6/SF7 golden-проверка
...
| SF (`sf_in`) | Chips в символе | Отсчётов на символ (`L`=16) | Пример Fs при BW500к |
|---|---:|---:|---:|
| SF5 (`0000`) | 32 | 512 | 8.000 МГц |
| SF6 (`0001`) | 64 | 1024 | 8.000 МГц |
| SF7 (`0010`) | 128 | 2048 | 8.000 МГц |
...
Фазовый аккумулятор DDS — 16 бит, I/Q — signed 16 bit.

**SF8..SF12 распознаются интерфейсом `sf_in`, но пока не реализованы.** При
L=16 точный (bit-exact против MATLAB golden) целочисленный шаг частоты
`speed_change = 65536/(2^SF * L^2)` существует только для SF5..SF7; для SF8 он
не делится на симметричный ±диапазон без потери половины LSB, а для SF9..SF12
получается уже дробным. Расширение потребует отдельного дробного аккумулятора
частоты в `formiration_chirp.vhd`/`phase_to_sample.vhd`, а не только новых
констант — до тех пор эти коды безопасно не формируют выход (как и раньше для
