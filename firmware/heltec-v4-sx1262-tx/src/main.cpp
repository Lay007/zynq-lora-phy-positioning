...
bool applyProfile(const RadioProfile& requestedProfile) {
  ...
  if (!radioOk(radio.setOutputPower(requestedProfile.outputPowerDbm), "setOutputPower")) {
    return false;
  }
...
void printHelp() {
  ...
  Serial.println("BOOT button is an emergency stop; RF starts only by serial command.");
}
...
void serviceButton() {
  ...
  if (currentState != previousButtonState && now - lastButtonChangeMs >= kButtonDebounceMs) {
    ...
    if (currentState == LOW) {
      transmitterRunning = false;
      Serial.println("OK transmitter stopped by button");
      updateDisplay("STOPPED");
    }
  }
}
...
void setup() {
  ...
  // Allow the Heltec V4 FEM rail and the SX1262 TCXO supply to settle.
  ...
  // ESP32-S3 GPIO-matrix changes away from the first SX1262 SPI transaction.
