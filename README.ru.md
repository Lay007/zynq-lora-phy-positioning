...
> В M2 уже реализованы потоковый fixed-point коррелятор, совместный timing/CFO
> и отдельный frequency-only оценщик, слепое обнаружение начала пакета,
> выравнивание сетки отсчётов, SFD, кадрирование, reset, грубая временная метка
> и дробный ToA. В M3 сгенерирован namespaced Verilog, ToA прошёл точную HDL
> cosimulation 8/8, а полный переносимый SF7/L=8 тракт от IQ до AXI
> синтезирован для `xc7z020clg400-2`. CLG400 overlay подключает его к RX1-потоку
> vendor AD9361 shell и переносит атомарные метаданные в gpreg PS. Открыты
> упаковка generated IP и проверка нового образа на плате.
