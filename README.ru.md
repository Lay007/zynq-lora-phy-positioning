...
> Состояние на 2026-09-19: модели M1/M2 и generated/portable RTL M3 готовы.
> M4/M5 уже включают реальный приём SF7/BW125/L=8 на CLG400: две серии
> при gain 25 дБ без клиппинга дали **90 попыток, 84 захвата, 81 CRC**,
> в том числе **79/79 при `grid_err=0`**. Это статистика захватов/CRC,
> а не калиброванная точность ToA или квалификационный PER.
> Условия и ограничения записаны в [журнале](docs/clg400-joint-grid-experiment.md);
> сырые каталоги этих серий локальные и здесь не опубликованы.
> Подготовка M6 разделяет `trace_rearm` и обнуляющий epoch `stream_reset`;
> двухпакетная RTL-регрессия проходит. Новый битстрим, непрерывная аппаратная
> серия, калибровка задержки и синхронный TDoA ещё требуют выполнения.
> Следующие шаги #28/#29: [критерии квалификации](docs/qualification-gates.md).
