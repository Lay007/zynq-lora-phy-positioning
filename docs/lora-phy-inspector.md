...
and its golden symbol sequence is defined for SF5..SF12 (each scaled to that
SF's valid symbol range). Stages 3–5 keep their fixed rates and return
`NOT VERIFIED` until exact production golden models are committed.
...
production sequence for any SF5..SF12 (parsed from the filename).
