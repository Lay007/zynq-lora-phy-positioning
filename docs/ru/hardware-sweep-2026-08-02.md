...
Установлена прошивка `zynq-lora-heltec-v4-sx1262-tx 0.1.1`. SX1262 отвечает,
внешний FEM удерживается в bypass, после загрузки `running=no`, а BOOT может
только остановить передачу. Пакеты с Heltec ещё не передавались: сначала нужно
подтвердить подключение антенны и маркировку PCB V4.2/V4.3.
