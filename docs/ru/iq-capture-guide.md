...
Тайм-аут IIO должен превышать время набора и передачи одного буфера. При
1 Мвыб/с набор 4 194 304 отсчётов сам по себе занимает более четырёх секунд,
поэтому по умолчанию скрипт использует тайм-аут 30 секунд.
