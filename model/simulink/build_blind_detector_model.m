...
lora_sim.set_function_script(dut+"/BlindDetector", [
    ...
    "    end"
    ""
    "    % Preamble first, on the newest " + preambleSymbols + " bins alone. Gating this on"
    ...
    "    if filled >= uint8(" + preambleSymbols + ")"
    "        early = history(" + (runLength-preambleSymbols+1) + ");"
    "        preambleOk = true;"
    "        for k = " + (runLength-preambleSymbols+1) + ":" + runLength
    "            preambleOk = preambleOk && withinTolerance(history(k), early);"
    "        end"
    "        preambleDetected = preambleOk;"
    "        preambleBin = early;"
    "        % mod(-bin, N): N is a power of two, so the wrap is a mask and"
    "        % bin 0 needs no special case (N & (N-1) == 0)."
    "        chipsToBoundary = wrapBin(uint16(" + n + ") - early);"
    "    end"
    ""
    "    if filled == uint8(" + runLength + ")"
    ...
    "            runOk = runOk && withinTolerance(history(k), reference);"
    "        end"
