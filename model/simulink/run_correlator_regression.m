...
for index = selected
    ...
    % Every symbol must carry the sample index of its own window start.
    expectedTimestamps = ((0:entry.windowSymbols-1).')* ...
        entry.samplesPerSymbol;
    timestampError = max(abs(result.symbolSampleCount(:)-expectedTimestamps));
    ...
    if timestampError > 0
        failures(end+1, 1) = sprintf( ...
            "%s: symbol sample counts are wrong by up to %g", ...
            caseId, timestampError); %#ok<AGROW>
    end
    if romError > 0
        failures(end+1, 1) = sprintf( ...
