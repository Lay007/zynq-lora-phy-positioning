...
for target = options.Targets
    ...
    command = sprintf('"%s" -mode batch -nojournal -nolog -source "%s" ' + ...
        "-tclargs ""%s"" ""%s"" ""%s"" ""%s"" %g %g %g %g ""%s""", ...
        options.VivadoPath, tclScript, sourceDirectory, definition.wrapper, ...
        definition.top, options.Part, options.ProbePeriodNs, ...
        options.PowerClockMHz, options.InputTogglePercent, ...
        options.JunctionTemperatureC, resultFile);
    if options.Verbose
        fprintf("Running: %s\n", command);
    end
