...
lora_sim.set_function_script(dut+"/ToaInterpolator", [
    ...
    "persistent logTable state magnitudeAReg magnitudeBReg magnitudeCReg"
    "persistent logAReg logBReg logCReg numeratorReg denominatorReg"
    "persistent dividendReg divisorReg remainderReg quotientReg bitIndex"
    "persistent negativeResult logPeakReg"
    "if isempty(state)"
    "    logTable = int32(" + tableLiteral + ");"
    "    state = uint8(0);"
    "    magnitudeAReg = uint32(0);"
    "    magnitudeBReg = uint32(0);"
    "    magnitudeCReg = uint32(0);"
    "    logAReg = int32(0);"
    "    logBReg = int32(0);"
    "    logCReg = int32(0);"
    "    numeratorReg = int32(0);"
    "    denominatorReg = int32(0);"
    "    dividendReg = uint32(0);"
    "    divisorReg = uint32(0);"
    "    remainderReg = uint64(0);"
    "    quotientReg = uint32(0);"
    "    bitIndex = uint8(0);"
    ...
    "        magnitudeAReg = magnitudeBefore;"
    "        magnitudeBReg = magnitudePeak;"
    "        magnitudeCReg = magnitudeAfter;"
    "        state = uint8(1);"
    "    end"
    "elseif state >= uint8(1) && state <= uint8(3)"
    ...
    "    end"
    "    logValue = approximateLog2(operand, logTable);"
    "    if state == uint8(1)"
    "        logAReg = logValue;"
    "        state = uint8(2);"
    ...
    "    numeratorReg = int32(" + (2^(options.FractionBits-1)) + ")*(logAReg-logCReg);"
    "    denominatorReg = logAReg-int32(2)*logBReg+logCReg;"
    "    state = uint8(5);"
    ...
    "    dividendReg = uint32(abs(numeratorReg));"
    "    divisorReg = uint32(abs(denominatorReg));"
    "    remainderReg = uint64(0);"
    "    quotientReg = uint32(0);"
    "    bitIndex = uint8(31);"
    "    negativeResult = xor(numeratorReg < int32(0), denominatorReg < int32(0));"
    "    if denominatorReg == int32(0)"
    ...
    "    nextRemainder = bitshift(remainderReg, 1);"
    "    inputBit = bitshift(dividendReg, -31);"
    "    nextRemainder = nextRemainder+uint64(inputBit);"
    "    dividendReg = bitshift(dividendReg, 1);"
    "    nextQuotient = bitshift(quotientReg, 1);"
    "    if nextRemainder >= uint64(divisorReg)"
    "        nextRemainder = nextRemainder-uint64(divisorReg);"
    "        nextQuotient = bitor(nextQuotient, uint32(1));"
    "    end"
    ...
    "        state = uint8(7);"
    "    else"
    "        bitIndex = bitIndex-uint8(1);"
    "    end"
    "else"
    "    % Saturation is deliberately a separate registered stage."
    "    if quotientReg > uint32(limit)"
    "        value = limit;"
    "    else"
    "        value = int32(quotientReg);"
    "    end"
    "    if negativeResult"
    "        value = -value;"
    "    end"
    ...
    "    if scanValue >= uint32(" + entries + ") && scanValue < uint32(" + (2*entries) + ")"
    "        shifted = scanValue;"
    "    end"
    "    scanValue = bitshift(scanValue, -1);"
    "    if scanValue > uint32(0)"
    "        exponent = bit;"
    "    end"
    "end"
    "% Mantissa: the " + options.LogTableBits + " bits below the leading one."
    "if value < uint32(" + entries + ")"
    "    shifted = value;"
    "    for bit = int32(1):int32(" + options.LogTableBits + ")"
    "        if shifted < uint32(" + entries + ")"
