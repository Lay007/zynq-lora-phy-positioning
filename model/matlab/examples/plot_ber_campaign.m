...
for index = 1:numel(targets)
    ...
    valid = isfinite(subset.LegacyGap_dB);
    legendHandles(end+1) = plot(axesHandle, ...
        subset.SamplesPerChip(valid), subset.LegacyGap_dB(valid), ...
    ...
    valid = isfinite(subset.CurrentGap_dB);
    legendHandles(end+1) = plot(axesHandle, ...
        subset.SamplesPerChip(valid), subset.CurrentGap_dB(valid), ...
        "--"+markers(index), "Color", colors(index, :), ...
        "LineWidth", 1.5, "MarkerSize", 6); %#ok<AGROW>
