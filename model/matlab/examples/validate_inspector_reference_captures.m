...
% LFS objects before running this function.
...
for index = 1:rowCount
    ...
    profile = lora_phy.match_lora_profile( ...
        result.estimatedSpreadingFactor, result.estimatedBandwidthHz, 0);

    sfOk = result.estimatedSpreadingFactor == item.expectedSf;
    bwOk = result.estimatedBandwidthHz == item.expectedBwHz;
    cfoOk = abs(result.estimatedCarrierOffsetHz-item.expectedCfoHz) <= 3000;
    ...
    cfoHz(index) = result.estimatedCarrierOffsetHz;
    ...
    pass(index) = sfOk && bwOk && cfoOk && snrOk && sourceCompatible;

    fprintf("Inspector %-20s: SF%d BW %.0f kHz CFO %+.1f Hz SNR %.1f dB -> %s\n", ...
        item.name, result.estimatedSpreadingFactor, ...
        result.estimatedBandwidthHz/1e3, result.estimatedCarrierOffsetHz, ...
        result.estimatedSnrDb, pass_fail(pass(index)));
end

report = table(caseName, sourceRadio, expectedSf, detectedSf, ...
    expectedBwHz, detectedBwHz, expectedCfoHz, cfoHz, snrDb, ...
    compatibleRadios, pass);
...
function item = make_case(name, sourceRadio, relativePath, fs, sf, bw, cfo)
item = struct( ...
    "name", string(name), ...
