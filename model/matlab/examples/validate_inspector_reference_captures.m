...
for index = 1:rowCount
    ...
    fprintf('Inspector %-20s: SF%d BW %.0f kHz carrier %+.1f kHz CFO %+.1f Hz SNR %.1f dB -> %s\n', ...
        char(item.name), result.estimatedSpreadingFactor, ...
        result.estimatedBandwidthHz/1e3, result.estimatedCarrierOffsetHz/1e3, ...
        measuredCfoHz, result.estimatedSnrDb, char(pass_fail(pass(index))));
end
